...
const { maskIPAddress } = require('./utils/security');
...
// Request logging middleware - مع إخفاء عناوين IP
app.use((req, res, next) => {
  const maskedIP = maskIPAddress(req.ip);
  console.log(`${new Date().toLocaleString('ar-IQ')} - ${req.method} ${req.path} - IP: ${maskedIP}`);
  next();
});
